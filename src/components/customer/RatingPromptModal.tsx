...
export const RatingPromptModal: React.FC<RatingPromptModalProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      await apiService.submitRating(booking.id, rating);
    ...
    >
      <View className="flex-1 justify-center items-center bg-black/80 px-6">
        <View className="w-full max-w-[340px] bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
          {/* Top Gold Gradient Light Effect */}
          <View className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-500" />
          ...
          <View className="bg-slate-950 border border-slate-800/80 rounded-2xl p-3 mb-5">
            ...
            {booking.service_date && (
              <View className="flex-row items-center mt-2">
                <Ionicons name="calendar-outline" size={12} color="#D4AF37" className="mr-1.5" />
          ...
          <View className="space-y-2.5">
            ...
            >
              {isLoading && actionType === 'submit' ? (
                ...
                <Text className={`font-black text-sm tracking-wider uppercase ${rating ? 'text-black' : 'text-slate-500'}`}>
                  Submit Review
                </Text>
              )}
            </Pressable>
