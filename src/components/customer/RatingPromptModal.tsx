...
export const RatingPromptModal: React.FC<
...
> = ({
  ...
  onSuccess,
    ...
    const serviceName = Array.isArray(
      booking.service_name
    )
      ? booking.service_name.join(', ')
      : typeof booking.service_name ===
        'string'
        ? booking.service_name
        : 'Premium Service';
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        logger.info(
          LogTag.API,
          `Successfully submitted review for booking: ${booking.id}`
        );
    ...
    const handleIgnore = async () => {
      ...
      try {
        await apiService.ignoreRating(
          booking.id
        );

        logger.info(
          LogTag.API,
          `Successfully ignored review for booking: ${booking.id}`
        );
          ...
          <View className="w-full max-w-[340px] bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
            ...
            <View className="space-y-2.5">
              <Pressable
                ...
                className={`h-12 w-full rounded-2xl items-center justify-center flex-row ${rating
                    ? 'bg-amber-500'
                    : 'bg-slate-800'
                  }`}
              >
                {isLoading &&
                  ...
                  <Text
                    className={`font-black text-sm tracking-wider uppercase ${rating
                        ? 'text-black'
                        : 'text-slate-500'
                      }`}
                  >
