...
function BookingScreenInner(): JSX.Element {
  ...
  const {
    selectedService,
    selectedServices: storeSelectedServices,
    resetBooking,
  } = useBookingStore();
  ...
  } = useSlots(id as string, selectedDate.dateString, activeService?.id);
      ...
      if (isRescheduling && bookingId) {
        ...
        router.replace({
          ...
          params: {
            ...
            salonImage: business.image_url || business.cover_photo_url || '',
          },
        });
      }
    } catch (err: any) {
        ...
        >
                ...
                {selectedServices.map((srv) => (
                  <View key={srv.id} className="flex-row items-center justify-between py-1">
                    <View className="flex-1 mr-4">
                      <Text className="text-slate-900 font-extrabold text-base mb-0.5">
                        {srv.name}
                      </Text>
                      ...
                      return (
                        <Pressable
                          ...
                          className={`px-4 py-2.5 rounded-2xl border ${
                            isSelected ? 'bg-black border-black' : 'bg-slate-50 border-slate-200'
                          }`}
                        >
                          <Text
              ...
              {availableDates.map((item, index) => {
                ...
                return (
                  <Pressable
                    ...
                    className={`w-16 h-24 rounded-3xl border items-center justify-center mr-3 ${
                      isSelected ? 'bg-black border-black' : 'bg-white border-slate-200'
                    }`}
                  >
                    <Text
                      className={`text-[10px] font-bold tracking-wider uppercase ${isSelected ? 'text-slate-400' : 'text-slate-400'}`}
                    >
                      {item.dayName}
                    </Text>
                    <Text
                      className={`text-xl font-black my-1 ${isSelected ? 'text-white' : 'text-slate-900'}`}
                    >
                      {item.dayNumber}
                    </Text>
                    <Text
                      className={`text-[9px] font-bold ${isSelected ? 'text-white' : 'text-slate-500'}`}
                    >
                      {item.monthName}
                    </Text>
                  </Pressable>
            ...
            <View className="px-luxury mt-6">
              ...
              {slotsLoading ? (
                <View className="bg-white border border-slate-200 rounded-3xl p-8 items-center justify-center">
                  ...
                  <Text className="text-slate-500 text-xs mt-2 font-bold">
                ...
                <View className="bg-white border border-slate-200 rounded-3xl p-8 items-center">
                  ...
                  <Text className="text-slate-500 text-sm mt-2 text-center font-semibold">
                  ...
                  {normalizedSlots.map((slot) => {
                    ...
                    return (
                      <Pressable
                        ...
                        className={`w-[30%] py-4 rounded-2xl border items-center justify-center ${
                          !isAvailable
                      ...
                      >
                        <Text
                          className={`text-sm font-bold ${isAvailable ? (isSelected ? 'text-white' : 'text-slate-900') : 'text-slate-400'}`}
                        >
                          {slot.label}
                        </Text>
                      </Pressable>
          ...
          <View className="px-luxury mt-6">
            ...
            >
              ...
              <View>
                <Text className="text-slate-600 text-xs font-bold uppercase mb-1.5">
                  Phone Number
                </Text>
                <View className="flex-row items-center border border-slate-200 rounded-2xl h-12 px-4 bg-slate-50">
