...
export const useBookings = (roleInput?: 'Customer' | 'Owner') => {
  ...
  useEffect(() => {
    if (query.isSuccess) {
      logger.info(LogTag.QUERY, `Bookings fetched for ${role || 'all'}. Count: ${query.data?.length}`);
    }
    if (query.isError) {
      logger.error(LogTag.QUERY, `Failed to fetch bookings for ${role || 'all'}`, query.error);
    }
  }, [query.isSuccess, query.isError, query.data, query.error, role]);
...
export const useUpdateBookingStatus = () => {
  return useMutation({
    ...
    onSuccess: (_: any, variables: { id: string; status: BookingStatus }) => {
      logger.info(LogTag.API, `✅ Booking status updated to ${variables.status}`, { id: variables.id });
      queryClient.invalidateQueries({ queryKey: queryKeys.bookings.all() });
