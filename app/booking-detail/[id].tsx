...
export default function BookingDetailScreen() {
    ...
    const fetchWhatsapp = async () => {
      try {
        ...
        const res = await apiClient.get(
          `/bookings/${targetBookingId}/whatsapp`
        );
  ...
  useEffect(() => {
    ...
    const fetchSlots = async () => {
      try {
        const salonId =
          booking?.business_id || booking?.business?.id;
        ...
        if (!cancelled) {
          const slotData = Array.isArray(res?.data)
            ? res.data
            : res?.data?.slots || [];
  ...
  const handleRescheduled = () => {
    router.push({
      ...
      params: {
        ...
        serviceIds: JSON.stringify(booking.services?.map((s: any) => s.id) || [booking.service?.id]),
          ...
          <AnimatedSection delay={50} className="mb-5">
            ...
            >
              <View className="flex-1 mr-2">
                <Text
                  className={`font-black text-lg ${statusConfig.text}`}
                >
                  {statusConfig.label}
                </Text>
            ...
            <View className="bg-white border border-slate-200 rounded-2xl p-5">
              ...
              <View className="flex-row items-center gap-x-4">
                ...
                {booking.customer_phone && (
                  <TouchableOpacity
                    onPress={() =>
                      handleCall(booking.customer_phone)
                    }
    ...
    <View className="flex-row items-center gap-x-4">
      <Avatar
        url={
          ...
          booking.business?.cover_photo_url ||
          null
        }
        name={
