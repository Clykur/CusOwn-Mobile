...
export default function GoogleCallbackScreen() {
  ...
  useEffect(() => {
    async function handleCallback() {
      ...
      if (error) {
        logger.error(LogTag.AUTH, `[CALLBACK] ❌ OAuth Error: ${error}`, new Error(error_description));
      ...
      if (session) {
        logger.info(LogTag.AUTH, '[CALLBACK] ✅ Session already exists, waiting for RootLayout to redirect...');
        return;
      }
      ...
      if (code) {
        try {
          logger.info(LogTag.AUTH, '[CALLBACK] 🔑 Exchanging code for session (Fallback handler)...');
          ...
          logger.warn(LogTag.AUTH, '[CALLBACK] ⚠️ Exchange failed or already handled:', err.message);
          
          // Double check if a session appeared while we were trying (race condition)
          const { data: { session: currentSession } } = await supabase.auth.getSession();
          if (currentSession) {
             logger.info(LogTag.AUTH, '[CALLBACK] 🔍 Session found after failed exchange, proceeding...');
             await setSession(currentSession);
             return;
          }

          // If it's a real error and we still don't have a session, go back
          if (!err.message?.includes('verifier not found') && !err.message?.includes('already used')) {
        ...
        const timeout = setTimeout(() => {
          if (!useAuthStore.getState().session) {
            logger.info(LogTag.AUTH, '[CALLBACK] ⏹️ No session after timeout, returning to welcome');
            router.replace('/(public)/welcome');
          }
        }, 3000);
